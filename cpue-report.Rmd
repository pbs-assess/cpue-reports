...
params:
   ...
   species_proper: "North Pacific Spiny Dogfish"
  ...
  if (params$discard_only) {
    ...
    glmmTMB::glmmTMB(as.formula(formula), data = dat, family = Gamma(link = "log"), 
      control = glmmTMB::glmmTMBControl(optCtrl = list(iter.max = 2000, 
        eval.max = 2000), profile = FALSE, collect = FALSE),  ...)
    }
  } else {
    dfleet[[1]]$cpue <- dfleet[[1]]$cpue > 0 
    fit_func <- function(dat, formula = cpue ~ year_factor, ...)  {
    glmmTMB::glmmTMB(as.formula(formula), data = dat, family = binomial(link = "logit"), 
      control = glmmTMB::glmmTMBControl(optCtrl = list(iter.max = 2000, 
        eval.max = 2000), profile = FALSE, collect = FALSE),  ...)
    }

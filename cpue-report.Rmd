...
for (i in seq_along(params$area_name)) {
  p <- make_fe_plots(model[[toplot[i]]]) + ggtitle(params$area_name[i])
  print(p)
}
```
...
```{r cpue-coef-plot3, fig.asp=0.45, echo=FALSE}
for (i in seq_along(params$area_name)) {
  p <- make_year_locality_plots(model[[toplot[i]]]) + ggtitle(params$area_name[i])
  print(p)
}
```

...
```{r, eval=FALSE, include=FALSE}
  # fe <- unique(d1996a$fishing_event_id_unique)
  # y <- filter(d1996, fishing_event_id_unique %in% fe)
  # 
  # group_by(y, species_common_name) %>% summarise(landed_kg = sum(landed_kg)) %>% arrange(-landed_kg) %>% top_n(10) -> .top
  # 
  # y$year <- lubridate::year(y$best_date)
  # y %>% filter(species_common_name %in% .top$species_common_name) %>% 
  #   group_by(species_common_name, year) %>% summarise(landed_kg = sum(landed_kg)) %>% 
  #   ggplot(aes(year, landed_kg)) + geom_col() +
  #   facet_wrap(~species_common_name)
  # ggsave("~/Downloads/fig.png")
```
